...
function MenuSection({
  ...
  return (
    <SectionReveal className="mb-24">
      ...
      <h2 className="text-3xl sm:text-4xl font-black text-white mb-10">{title}</h2>
      <div className={`grid grid-cols-1 ${cols === 3 ? "md:grid-cols-3" : "md:grid-cols-2"} gap-5`}>
        {items.map((item, idx) => (
          ...
          >
            ...
            <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.40)" }}>{item.description}</p>
          </div>
        ))}
      </div>
    </SectionReveal>
